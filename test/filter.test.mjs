...
import { ProjectFilters, isHiddenRootSection, supportsRootFilter } from "../out/filter.js";
...
test("only proven empty configuration root sections are hidden", () => {
  for (const type of ["configuration", "extension", "report", "processing"]) {
    ...
    for (const state of ["empty", "non_empty", "unloaded", "error"]) {
      for (const rootSection of [true, false]) {
        const entry = { project, node: { state, rootSection } };
        assert.equal(isHiddenRootSection(entry, true), supportsRootFilter(project) && rootSection && state === "empty");
        assert.equal(isHiddenRootSection(entry, false), false);
      }
    }
  }
});
...
test("real root filters retain nested groups and follow last-object changes without reloading on toggle", { skip: !executable }, async t => {
  ...
  for (const type of ["configuration", "extension", "report", "processing"]) {
    ...
    const shown = all.filter(entry => !isHiddenRootSection(entry, true));
    ...
    if (supportsRootFilter(top.project)) {
      ...
      assert.ok(commonChildren.every(entry => !isHiddenRootSection(entry, true)), "nested common groups stay visible");
      ...
      assert.ok(groups.every(entry => !isHiddenRootSection(entry, true)), "empty object collections stay visible");
      ...
      assert.equal(isHiddenRootSection(named(removed, "Справочники"), true), true);
      await writeFile(rootFile, xml);
      await tree.refresh(top.project);
      await tree.root(top.project);
      assert.equal(isHiddenRootSection(named(await tree.children(top), "Справочники"), true), false);
    } else {
...
test("workspace members retain independent filters after reconnect", { skip: !executable }, async t => {
  ...
  for (let run = 0; run < 2; run++) {
    ...
    for (const entry of [first, second]) {
      ...
      const shown = (await tree.children(entry)).filter(child => !isHiddenRootSection(child, hide));
