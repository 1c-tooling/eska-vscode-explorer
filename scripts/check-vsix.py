...
def check(archive):
    ...
    with zipfile.ZipFile(archive) as package:
        ...
        required = ['package.json', 'package.nls.json', 'package.nls.ru.json', 'README.md', 'CHANGELOG.md']
        ...
        expected = {'extension/' + name for name in required} | {'extension.vsixmanifest', '[Content_Types].xml'}
        ...
        for name in required:
            if name != 'package.json':
                assert package.read(normalized[('extension/' + name).lower()]) == (root / name).read_bytes(), name
