...
import { bootstrapCommand, checkUpdate, compareVersions, inspectGlobal, latestRelease, type InstallCommand } from "./installation.js";
...
export class BackendSetup implements vscode.Disposable {
  ...
  async ensure(): Promise<string | undefined> {
    ...
    const cli = await inspectGlobal(this.log);
    ...
    if (!choice || this.disposed) return undefined;
    if (cli && compareVersions(release.version, cli.version) <= 0) throw new ExplorerError("incompatible");
    await this.perform(await bootstrapCommand(cli, release));
    const installed = await inspectGlobal(this.log);
  ...
  async check(manual: boolean): Promise<void> {
    ...
    try {
      ...
      const release = cli.selfUpdate ? undefined : await latestRelease();
      const available = release ? release.version : (await checkUpdate(cli)).version;
      ...
      const command: InstallCommand = !release
        ...
        : await bootstrapCommand(cli, release);
  ...
  private async perform(command: InstallCommand): Promise<void> {
  ...
  /** Deactivation terminates only the task launched by this extension. */
  dispose(): void { this.disposed = true; this.execution?.terminate(); }
}
