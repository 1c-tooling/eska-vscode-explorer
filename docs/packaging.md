...
Версия берётся из `package.json`: `0.0.0` — текущая тестовая сборка, а не опубликованный
релиз. Версию меняет существующая release automation. VSIX не содержит backend eska.
...
2. В меню Extensions выберите **Install from VSIX…**, укажите файл
   `eska-explorer-0.0.0.vsix` и при необходимости перезагрузите окно.
...
Также можно выполнить `code --install-extension /полный/путь/eska-explorer-0.0.0.vsix`.
...
bun run package
python3 scripts/check-vsix.py eska-explorer-0.0.0.vsix
```
...
упаковку. `.vscodeignore` разрешает только runtime JS, NLS, документацию и SVG.
...
Лицензия проекта пока не выбрана; локальная упаковка использует `--skip-license`,
не добавляя лицензию от имени автора. GitHub Actions, публикация, теги и версия
этой командой не изменяются. Python нужен только для проверки архива.
